...
async function joystickMove(direction) { const speed = parseInt($("speed-input").value, 10) || selectedSpeed; document.querySelectorAll(".joy-btn").forEach(b => b.classList.remove("active")); const btn = document.querySelector('.joy-btn[data-dir="' + direction + '"]'); if (btn) btn.classList.add("active"); if (!movementPolling) startMovementTracking(); try { await fetch("/api/joystick/move", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ direction, speed }) }); } catch (e) {} }
async function joystickStop() { document.querySelectorAll(".joy-btn").forEach(b => b.classList.remove("active")); stopMovementTracking(); try { await fetch("/api/joystick/stop", { method: "POST" }); } catch (e) {} pollPosition(); }
